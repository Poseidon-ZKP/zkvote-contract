...
contract Nouns is IDAOProxy {
    ...
    function add_voter(
        ...
        require(
            registered_voting_power[proposalId] <=
                zkVote.maxTotalVotingWeight(),
            "total voting power exceeded"
        );
    }

    function get_voting_weight(
        uint256 proposalId,
        address voter
    ...
    function receiveVoteTally(
        ...
        uint256 againstVotes,
        uint256 abstainVotes
    ) public override onlyZKVote {
        voteTallies[proposalId] = VoteTally(
            forVotes,
            againstVotes,
            abstainVotes
        );
    ...
    function setupVote(uint256 proposalId, uint256 endBlock) public {
    ...
    modifier onlyZKVote() {
        require(
            msg.sender == address(zkVote),
            "Nouns::onlyZKVote: Only ZKVote can call this function."
        );
        _;
    }
}
